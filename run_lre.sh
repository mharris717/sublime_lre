...
function run_lre() {
  ...
  if [[ $? -eq 0 ]] ; then
    ...
    printf "Failed to change into project root directory '$1', lre could not be started.\n\n"
  fi
}

echo "Starting Lre for $1"
...
if [ $? -ne 0 ]; then # failed to load rvm
  ...
  printf "Attempting to run Lre using your system gemset.\n"
  run_lre "$1"
  ...
  if [ -e "$1/.rvmrc" ]; then # found project specific .rvmrc
    ...
    if [ $? -ne 0 ]; then # failed to load project specific .rvmrc
      ...
      printf "\nWE DID NOT RUN LRE AS LOADING YOUR PROJECT SPECIFIC .rvmrc SHOULD NOT HAVE FAILED\n"
      return 1
    ...
    if [ $? -ne 0 ]; then # failed to switch to the users default gemset
      ...
      printf "\nWE DID NOT RUN LRE AS SWITCHING TO YOUR DEFAULT RVM ENVIRONMENT SHOULD NOT HAVE FAILED\n"
      return 1
